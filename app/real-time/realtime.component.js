		...
		function RealTimeController(ServicesService, $interval, $scope) {
			...
			function parseData(service, data) {
				...
				jQuery.each(buckets, function(i, loglevel) {
					if (loglevel.datetime != undefined && loglevel.datetime.buckets != undefined) {
						jQuery.each(loglevel.datetime.buckets, function(j, datetime) {
							var k = loglevel.key == 'info' ? 1 : loglevel.key == 'error' ? 0 : 2
							if (j == 0)
								values[k][0] = datetime.doc_count;
							else values[k].push(datetime.doc_count);
						});
